...
class ModelService:
    ...
    @staticmethod
    def _nullable_decimal_to_float(value) -> Optional[float]:
        """Serialize an optional price without collapsing NULL and zero."""
        return None if value is None else float(value)

    @staticmethod
    def _normalize_cache_read_price(value: object) -> Optional[Decimal]:
        """Normalize a model-specific cache read price while preserving NULL."""
        if value is None or value == "":
            return None
        try:
            amount = Decimal(str(value)).quantize(ModelService.PRICE_QUANTUM)
        except (InvalidOperation, TypeError, ValueError):
            raise ServiceException(400, "缓存读取价格参数无效", "INVALID_CACHE_READ_PRICE")
        if not amount.is_finite():
            raise ServiceException(400, "缓存读取价格参数无效", "INVALID_CACHE_READ_PRICE")
        if amount < Decimal("0"):
            raise ServiceException(400, "缓存读取价格不能小于 0", "INVALID_CACHE_READ_PRICE")
        return amount

    @staticmethod
    def resolve_cache_read_price(model: object) -> Decimal:
        """Resolve the effective cache-read price for the current model.

        An unset model-specific price intentionally follows the current input
        price at billing time. An explicit zero remains free.
        """
        input_price = Decimal(str(getattr(model, "input_price_per_million", 0) or 0))
        configured_price = getattr(model, "cache_read_price_per_million", None)
        if configured_price is None:
            configured_price = input_price * ModelService.CACHE_READ_FALLBACK_RATE
        return Decimal(str(configured_price)).quantize(ModelService.PRICE_QUANTUM)
    ...
    def _model_to_dict(model: UnifiedModel) -> dict:
        ...
        return {
            ...
            "cache_read_price_per_million": ModelService._nullable_decimal_to_float(
                getattr(model, "cache_read_price_per_million", None)
            ),
    ...
    def create_model(db: Session, data) -> dict:
        ...
        cache_read_price = ModelService._normalize_cache_read_price(
