...
class ProxyService:
    ...
    def _build_text_quota_precheck(
        ...
        if unified_model is not None:
            ...
            quota_precheck["global_price_multiplier_snapshot"] = global_price_multiplier
            quota_precheck["adjustment_price_multiplier_snapshot"] = adjustment_price_multiplier
    ...
    async def _stream_responses_request(
        ...
        billing_context = billing_context or ProxyService._build_text_billing_context("responses", request_data)
    ...
    def _deduct_balance_and_log_once(
        ...
        with session_scope() as write_db:
            ...
            frozen_adjustment_multiplier = billing_context.get("adjustment_price_multiplier_snapshot")
            frozen_global_multiplier = billing_context.get("global_price_multiplier_snapshot")
            if frozen_adjustment_multiplier is not None and frozen_global_multiplier is not None:
                global_price_multiplier = Decimal(str(frozen_global_multiplier))
                adjustment_price_multiplier = Decimal(str(frozen_adjustment_multiplier)).quantize(Decimal("0.000001"))
                price_adjustment_source = billing_context.get("price_adjustment_source_snapshot") or "default"
                price_adjustment_rule_id = billing_context.get("price_adjustment_rule_id_snapshot")
            else:
            ...
            write_db.add(
                ConsumptionRecord(
                    ...
                    adjustment_price_multiplier_snapshot=adjustment_price_multiplier,
    ...
    def _log_failed_request(
        ...
        try:
            with session_scope() as write_db:
                ...
                context = billing_context or {}
                global_price_multiplier = Decimal(str(context.get("global_price_multiplier_snapshot") or "1"))
                adjustment_price_multiplier = Decimal(
                    str(context.get("adjustment_price_multiplier_snapshot") or "1")
                ).quantize(Decimal("0.000001"))
                price_adjustment_source = context.get("price_adjustment_source_snapshot") or "default"
                price_adjustment_rule_id = context.get("price_adjustment_rule_id_snapshot")
                context_tokens_snapshot = int(Decimal(str(context.get("context_tokens_snapshot") or "0")))
                context_token_threshold_snapshot = int(
                    Decimal(str(context.get("context_token_threshold_snapshot") or ProxyService._LONG_CONTEXT_TOKEN_THRESHOLD))
                )
                context_price_multiplier = Decimal(
                ...
                write_db.add(
                    RequestLog(
                        ...
                        price_adjustment_source_snapshot=price_adjustment_source,
                        price_adjustment_rule_id_snapshot=price_adjustment_rule_id,
                        price_multiplier_snapshot=price_multiplier,
    ...
    def _store_video_task_route(
        ...
        *,
        ...
        try:
            with session_scope() as write_db:
                snapshot = (
                    write_db.query(VideoTaskBillingSnapshot)
                    .filter(VideoTaskBillingSnapshot.video_id == str(video_id))
                    .first()
                )
                if snapshot is None:
                    snapshot = VideoTaskBillingSnapshot(video_id=str(video_id))
                ...
                snapshot.charged_credits = Decimal(str(charged_credits or "0")).quantize(Decimal("0.001"))
                ...
                snapshot.price_adjustment_source_snapshot = price_adjustment_source or "default"
                snapshot.price_adjustment_rule_id_snapshot = int(price_adjustment_rule_id) if price_adjustment_rule_id else None
                if billed:
                    snapshot.billed = 1
                    snapshot.billed_at = datetime.utcnow()
                elif not snapshot.billed:
                    snapshot.billed = 0
                    snapshot.billed_at = None
        except Exception as exc:
            logger.error("Failed to persist video task billing snapshot video_id=%s: %s", video_id, exc, exc_info=True)

    @staticmethod
    def _load_video_task_route_snapshot(db: Session, user: SysUser, video_id: str) -> Optional[dict[str, Any]]:
        user_id = ProxyService._safe_object_id(user)
        if not user_id:
            return None
        snapshot = (
            db.query(VideoTaskBillingSnapshot)
            .filter(
                VideoTaskBillingSnapshot.video_id == str(video_id),
                VideoTaskBillingSnapshot.user_id == user_id,
            )
            .first()
        )
        if not snapshot:
        ...
        route_payload = {
            ...
            "requested_model": snapshot.requested_model or "",
            ...
            "billed": bool(snapshot.billed),
        }
        ProxyService._video_task_routes[str(video_id)] = route_payload
        return route_payload

    @staticmethod
    def _mark_video_task_snapshot_billed(video_id: str) -> None:
        if not video_id:
            return
        try:
            with session_scope() as write_db:
            ...
            logger.error("Failed to mark video task snapshot billed video_id=%s: %s", video_id, exc, exc_info=True)
    ...
    @staticmethod
    def _is_cpa_grok_video_channel(channel: Optional[Channel]) -> bool:
        return str(getattr(channel, "provider_variant", "") or "").strip().lower() == ProxyService._CPA_GROK_VIDEO_VARIANT

    @staticmethod
    def _is_grok_video_119337_channel(channel: Optional[Channel]) -> bool:
        return str(getattr(channel, "provider_variant", "") or "").strip().lower() == ProxyService._GROK_VIDEO_119337_VARIANT
    ...
    def _resolve_cpa_grok_video_create_url(base_url: str) -> str:
        normalized = str(base_url or "").rstrip("/")
        if normalized.endswith("/v1"):
            return f"{normalized}/videos/generations"
        return f"{normalized}/v1/videos/generations"

    @staticmethod
    def _resolve_cpa_grok_video_retrieve_url(base_url: str, video_id: str) -> str:
        normalized = str(base_url or "").rstrip("/")
        if normalized.endswith("/v1"):
            return f"{normalized}/videos/{video_id}"
        return f"{normalized}/v1/videos/{video_id}"

    @staticmethod
    def _resolve_grok_video_119337_create_url(base_url: str) -> str:
        normalized = str(base_url or "").rstrip("/")
        if normalized.endswith("/v1"):
            return f"{normalized}/video/generations"
        return f"{normalized}/v1/video/generations"

    @staticmethod
    def _resolve_grok_video_119337_retrieve_url(base_url: str, video_id: str) -> str:
        return f"{ProxyService._resolve_grok_video_119337_create_url(base_url)}/{video_id}"
    ...
    @staticmethod
    def _normalize_grok_video_119337_seconds(seconds: int) -> int:
        supported_seconds = (4, 6, 8, 10, 12, 15)
        if seconds <= supported_seconds[0]:
            return supported_seconds[0]
        if seconds >= supported_seconds[-1]:
    ...
    @staticmethod
    def _count_video_reference_inputs(request_data: dict) -> int:
        references = request_data.get("input_references")
        if not isinstance(references, list):
            return 0
        return sum(1 for item in references if isinstance(item, dict) and item.get("content"))

    @staticmethod
    def _video_reference_to_data_url(reference_file: dict[str, Any]) -> str:
        content = reference_file.get("content")
    ...
    @staticmethod
    def _cpa_grok_video_aspect_ratio(size: str) -> str:
        normalized = str(size or "").strip().lower()
        if normalized in {"720x1280", "1024x1792"}:
            return "9:16"
        if normalized in {"1280x720", "1792x1024"}:
            return "16:9"
    ...
    @staticmethod
    def _grok_video_119337_aspect_ratio(size: str) -> str:
        normalized = str(size or "").strip().lower()
        if normalized in {"720x1280", "1024x1792"}:
            return "9:16"
        if normalized in {"1024x1024"}:
    ...
    @staticmethod
    def _normalize_cpa_grok_video_status(status: Any) -> str:
    ...
    @staticmethod
    def _normalize_grok_video_119337_status(status: Any) -> str:
        normalized = str(status or "").strip().lower()
        if normalized in {"success", "succeeded", "completed", "done"}:
            return "completed"
        if normalized in {"failure", "failed", "error", "cancelled", "canceled", "expired"}:
            return "failed"
        if normalized in {"in_progress", "processing", "running"}:
            return "in_progress"
        if normalized in {"submitted", "queued", "not_start", "pending"}:
            return "queued"
        return normalized or "queued"

    @staticmethod
    def _grok_video_119337_body_data(body: dict[str, Any]) -> dict[str, Any]:
        data = body.get("data")
        return data if isinstance(data, dict) else body

    @staticmethod
    def _extract_grok_video_119337_url(body: dict[str, Any]) -> str:
        data = body.get("data")
        value = data.get("result_url") if isinstance(data, dict) else None
        if isinstance(value, str) and value.strip():
            return value.strip()
        return ""

    @staticmethod
    def _normalize_grok_video_119337_response(
        body: dict[str, Any],
        *,
        ...
        video_url = ProxyService._extract_grok_video_119337_url(body)
        if video_url:
        ...
        if fail_reason:
            ...
            if status not in ProxyService._VIDEO_COMPLETED_STATUSES:
                normalized["status"] = "failed"
        elif isinstance(data.get("error"), (dict, str)):
    ...
    @staticmethod
    def _normalize_cpa_grok_video_response(
        body: dict[str, Any],
        *,
        video_id: Optional[str] = None,
        model: Optional[str] = None,
        ...
        status = ProxyService._normalize_cpa_grok_video_status(body.get("status"))
        normalized: dict[str, Any] = {
            "object": "video",
            "id": resolved_id,
            "model": model or body.get("model") or "",
        ...
        if seconds is not None:
            ...
            if video.get("duration") is not None:
                normalized["seconds"] = str(video.get("duration"))
        if size is not None:
            normalized["size"] = size
        elif body.get("size") is not None:
            normalized["size"] = body.get("size")
        if body.get("created_at") is not None:
            normalized["created_at"] = body.get("created_at")
        if body.get("completed_at") is not None:
            normalized["completed_at"] = body.get("completed_at")
        video_url = ProxyService._extract_cpa_grok_video_url(body)
        if video_url:
            normalized["video_url"] = video_url
        if isinstance(body.get("error"), (dict, str)):
            normalized["error"] = body.get("error")
    ...
    def _apply_media_credit_price_adjustment(
        ...
        adjustment_multiplier = PriceAdjustmentService.resolve_multiplier(db, unified_model, user_id=user_id)
        return (
            Decimal(str(base_multiplier or 0)) * adjustment_multiplier
        ).quantize(Decimal("0.001"))

    @staticmethod
    def _resolve_media_credit_price_adjustment(
        db: Session,
        unified_model: UnifiedModel,
        base_multiplier: Decimal,
        user_id: Optional[int] = None,
    ) -> tuple[Decimal, Any]:
        adjustment = PriceAdjustmentService.resolve_adjustment(db, unified_model, user_id=user_id)
        adjusted = (
            Decimal(str(base_multiplier or 0)) * adjustment.multiplier
        ).quantize(Decimal("0.001"))
    ...
    def _resolve_adjusted_video_credit_cost(
        ...
        adjusted_rate, adjustment = ProxyService._resolve_media_credit_price_adjustment(
            db,
            unified_model,
            rate,
            user_id=user_id,
    ...
    @staticmethod
    async def _non_stream_cpa_grok_video_request(
        db: Session,
        user: SysUser,
        api_key_record: UserApiKey,
        channel: Channel,
        ...
        if reference_files:
            if len(reference_files) > 1:
                payload["reference_images"] = [
                    {"url": ProxyService._video_reference_to_data_url(reference)}
                    for reference in reference_files
                ]
            else:
                payload["image"] = {
                    "url": ProxyService._video_reference_to_data_url(reference_files[0])
                }

        url = ProxyService._resolve_cpa_grok_video_create_url(channel.base_url)
        headers = ProxyService._build_headers(channel, "openai", request_headers=request_headers)
        ...
        response = await ProxyService._post_with_retries(
            ...
            channel=channel,
            timeout=timeout,
            log_label="CPA Grok video create",
        )
        if not 200 <= response.status_code < 300:
            body_text = response.text[:1000]
            upstream_detail = f"CPA Grok 视频任务创建失败（HTTP {response.status_code}）：{body_text}"
        ...
        if not isinstance(response_body, dict):
            raise ServiceException(
                503,
                "CPA Grok 视频任务创建响应格式无效",
                "OPENAI_VIDEO_GENERATION_FAILED",
            )
        video_id = str(response_body.get("request_id") or response_body.get("id") or "").strip()
        if not video_id:
            raise ServiceException(
                503,
                "CPA Grok 视频任务创建成功，但未返回 video_id",
                "OPENAI_VIDEO_GENERATION_FAILED",
            )
    ...
    @staticmethod
    async def _non_stream_grok_video_119337_request(
        db: Session,
        user: SysUser,
        api_key_record: UserApiKey,
        channel: Channel,
        ...
        if reference_files:
            payload["image_urls"] = [
                ProxyService._video_reference_to_data_url(reference)
                for reference in reference_files
            ]

        url = ProxyService._resolve_grok_video_119337_create_url(channel.base_url)
        headers = ProxyService._build_headers(channel, "openai", request_headers=request_headers)
        timeout = httpx.Timeout(_VIDEO_UPSTREAM_TIMEOUT, connect=_UPSTREAM_CONNECT_TIMEOUT)
        response = await ProxyService._post_with_retries(
            url,
            ...
            channel=channel,
            timeout=timeout,
            log_label="119337 Grok video create",
        )
        if not 200 <= response.status_code < 300:
            body_text = response.text[:1000]
            upstream_detail = f"119337 Grok 视频任务创建失败（HTTP {response.status_code}）：{body_text}"
            raise ProxyService._build_user_visible_upstream_request_error(
                "OPENAI_VIDEO_GENERATION_FAILED",
                upstream_detail=upstream_detail,
                status_code=response.status_code,
        ...
        try:
            ...
            raise ServiceException(
                503,
                "119337 Grok 视频任务创建响应解析失败",
                "OPENAI_VIDEO_GENERATION_FAILED",
            ) from exc
        if not isinstance(response_body, dict):
            raise ServiceException(
                503,
                "119337 Grok 视频任务创建响应格式无效",
                "OPENAI_VIDEO_GENERATION_FAILED",
            )
        response_data = ProxyService._grok_video_119337_body_data(response_body)
        video_id = str(
        ...
        if not video_id:
            raise ServiceException(
                503,
                "119337 Grok 视频任务创建成功，但未返回 task_id",
                "OPENAI_VIDEO_GENERATION_FAILED",
            )
        ...
        normalized_body = ProxyService._normalize_grok_video_119337_response(
            response_body,
            video_id=video_id,
            model=upstream_model_name,
            prompt=prompt,
        ...
        if bill_on_create:
            try:
                ...
                logger.error("119337 Grok video billing / logging failed after upstream success: %s", exc)
                raise ServiceException(
                    500,
                    "视频任务创建成功，但本地计费或记账失败，系统已中断返回，请稍后重试",
                    "VIDEO_BILLING_FAILED",
            ...
            try:
                ...
                if ProxyService._is_grok_video_119337_channel(channel):
                    return await ProxyService._run_with_billing_concurrency(
                        ...
                        admission_decision,
                        request_id,
                        lambda: ProxyService._non_stream_grok_video_119337_request(
                            db,
                            user,
                            api_key_record,
                            channel,
    ...
    @staticmethod
    def _sanitize_video_task_response_body(body: dict[str, Any], video_id: str) -> dict[str, Any]:
        sanitized = copy.deepcopy(body)
        sanitized["content_url"] = f"/v1/videos/{video_id}/content"
        sanitized["retrieve_url"] = f"/v1/videos/{video_id}"
    ...
    async def handle_video_request_and_wait(
        ...
        route_info = ProxyService._video_task_routes.get(str(video_id)) or {}
        normalized_seconds = int(
            route_info.get("video_seconds")
            or ProxyService._normalize_video_seconds(video_config.get("seconds"), channel)
        )
        normalized_size = str(
            route_info.get("video_size")
            or ProxyService._normalize_video_size(video_config.get("size"), channel)
        )
        billing_type = str(unified_model.billing_type or "image_credit")
        ...
        if model_multiplier <= Decimal("0") or (billing_type == "image_credit" and charged_credits <= Decimal("0")):
            model_multiplier, resolved_video_credit_cost, _price_adjustment = ProxyService._resolve_adjusted_video_credit_cost(
                db,
                unified_model,
                normalized_seconds,
                user_id=ProxyService._safe_object_id(user),
        ...
        try:
            await ProxyService._validate_video_route_content_or_raise(
                channel,
                video_id,
                request_headers=request_headers,
            )
            billed_now = ProxyService._bill_video_route_if_needed(
                db,
                user,
                api_key_record,
                video_id,
        ...
        response_body["final_status"] = ProxyService._sanitize_video_task_response_body(final_status, video_id)
        response_body = ProxyService._sanitize_video_task_response_body(response_body, video_id)
        response_body["usage"] = {
            **(response_body.get("usage") if isinstance(response_body.get("usage"), dict) else {}),
            "billing_type": billing_type,
            "image_credits_charged": float(charged_credits if billing_type == "image_credit" else Decimal("0.000")),
            ...
            "size": normalized_size,
            "seconds": normalized_seconds,
            "billing_status": "charged" if billed_now or ProxyService._video_success_log_exists(db, user, video_id) else "pending_completion",
        }
        response_body["wait"] = {
            "completed": True,
    ...
    def _resolve_stored_video_route(
        ...
        if not route:
            route = ProxyService._load_video_task_route_snapshot(db, user, video_id)
        if not route:
    ...
    def _resolve_video_route_from_request_log(
        ...
        ProxyService._store_video_task_route(
            ...
            billing_type=request_log.billing_type,
    ...
    def _bill_video_route_if_needed(
        ...
        *,
        ...
        if route_info is None:
            route_info = ProxyService._load_video_task_route_snapshot(db, user, video_id)
        ...
        adjustment_multiplier = (
            Decimal(str(route_info.get("adjustment_multiplier"))).quantize(Decimal("0.000001"))
            if route_info.get("adjustment_multiplier") is not None
            else None
        )
        price_adjustment_source = route_info.get("price_adjustment_source")
        price_adjustment_rule_id = (
            int(route_info.get("price_adjustment_rule_id"))
            if route_info.get("price_adjustment_rule_id")
            else None
        )
    ...
    @staticmethod
    async def _request_cpa_grok_video_status(
        channel: Channel,
        video_id: str,
        *,
        request_headers: Optional[dict[str, str]] = None,
    ) -> dict[str, Any]:
        url = ProxyService._resolve_cpa_grok_video_retrieve_url(channel.base_url, video_id)
        headers = ProxyService._build_headers(channel, "openai", request_headers=request_headers)
        headers.pop("Content-Type", None)
        timeout = httpx.Timeout(_VIDEO_UPSTREAM_TIMEOUT, connect=_UPSTREAM_CONNECT_TIMEOUT)
        async with httpx.AsyncClient(timeout=timeout) as client:
    ...
    @staticmethod
    async def _request_grok_video_119337_status(
        channel: Channel,
        video_id: str,
        *,
        request_headers: Optional[dict[str, str]] = None,
    ) -> dict[str, Any]:
        url = ProxyService._resolve_grok_video_119337_retrieve_url(channel.base_url, video_id)
        headers = ProxyService._build_headers(channel, "openai", request_headers=request_headers)
        headers.pop("Content-Type", None)
        timeout = httpx.Timeout(_VIDEO_UPSTREAM_TIMEOUT, connect=_UPSTREAM_CONNECT_TIMEOUT)
        try:
        ...
        try:
            ...
            raise ServiceException(
                503,
                "119337 Grok 视频任务查询响应解析失败",
                "OPENAI_VIDEO_RETRIEVE_FAILED",
            ) from exc
        if not isinstance(body, dict):
            raise ServiceException(
                503,
                "119337 Grok 视频任务查询响应格式无效",
                "OPENAI_VIDEO_RETRIEVE_FAILED",
            )
        return body

    @staticmethod
    async def _resolve_cpa_grok_video_download_url(
        channel: Channel,
        video_id: str,
        *,
        request_headers: Optional[dict[str, str]] = None,
    ) -> str:
        body = await ProxyService._request_cpa_grok_video_status(
            channel,
            video_id,
            request_headers=request_headers,
        )
        normalized = ProxyService._normalize_cpa_grok_video_response(body, video_id=video_id)
        ...
        if not ProxyService._is_video_completed_status(status):
            raise ProxyService._attach_upstream_detail(
                ...
                f"CPA Grok 视频任务尚未完成：video_id={video_id} status={status}",
            )
        video_url = ProxyService._extract_cpa_grok_video_url(body) or str(normalized.get("video_url") or "").strip()
        if not video_url:
            raise ProxyService._attach_upstream_detail(
                ServiceException(
                    503,
                ...
                f"CPA Grok 视频任务已完成但未返回 video.url：video_id={video_id}",
            )
        return video_url

    @staticmethod
    async def _resolve_grok_video_119337_download_url(
        channel: Channel,
        video_id: str,
        *,
        request_headers: Optional[dict[str, str]] = None,
    ) -> str:
        body = await ProxyService._request_grok_video_119337_status(
            channel,
            video_id,
            request_headers=request_headers,
        )
        normalized = ProxyService._normalize_grok_video_119337_response(body, video_id=video_id)
        status = ProxyService._video_status_value(normalized)
        if not ProxyService._is_video_completed_status(status):
            raise ProxyService._attach_upstream_detail(
                ServiceException(
                ...
                f"119337 Grok 视频任务尚未完成：video_id={video_id} status={status}",
            )
        video_url = ProxyService._extract_grok_video_119337_url(body) or str(normalized.get("video_url") or "").strip()
        if not video_url:
            raise ProxyService._attach_upstream_detail(
                ServiceException(
                    503,
                ...
                f"119337 Grok 视频任务已完成但未返回 data.result_url：video_id={video_id}",
            )
        return video_url

    @staticmethod
    async def _validate_video_route_content_or_raise(
        channel: Channel,
        video_id: str,
        *,
        request_headers: Optional[dict[str, str]] = None,
        ...
        if ProxyService._is_cpa_grok_video_channel(channel):
            video_url = await ProxyService._resolve_cpa_grok_video_download_url(
                channel,
                video_id,
                request_headers=request_headers,
            )
    ...
    async def _request_video_route(
        ...
        *,
        ...
        headers = ProxyService._build_headers(channel, "openai", request_headers=request_headers)
        if content and not ProxyService._should_send_video_url_auth(url, channel):
            headers = {}
        headers.pop("Content-Type", None)
        timeout = httpx.Timeout(_VIDEO_UPSTREAM_TIMEOUT, connect=_UPSTREAM_CONNECT_TIMEOUT)
        async with httpx.AsyncClient(timeout=timeout) as client:
    ...
    async def _stream_video_content_route(
        ...
        *,
        ...
        headers = ProxyService._build_headers(channel, "openai", request_headers=request_headers)
        if not ProxyService._should_send_video_url_auth(url, channel):
            headers = {}
        headers.pop("Content-Type", None)
        timeout = httpx.Timeout(_VIDEO_UPSTREAM_TIMEOUT, connect=_UPSTREAM_CONNECT_TIMEOUT)
    ...
    async def handle_video_retrieve(
        ...
        status = ProxyService._video_status_value(response_body)
        if ProxyService._is_video_completed_status(status):
            try:
                await ProxyService._validate_video_route_content_or_raise(
                    channel,
                ...
                logger.error("Video retrieve billing / logging failed after completion: %s", exc, exc_info=True)
                raise ServiceException(
                    500,
                    "视频任务已完成，但本地计费或记账失败，系统已中断返回，请稍后重试",
                    "VIDEO_BILLING_FAILED",
                ) from exc
        return JSONResponse(content=ProxyService._sanitize_video_task_response_body(response_body, video_id))
    ...
    async def handle_image_request(
        ...
        image_size, base_model_multiplier = ProxyService._resolve_image_billing_rule(db, unified_model, request_data)
