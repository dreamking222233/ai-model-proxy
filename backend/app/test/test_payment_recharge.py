...
class PaymentRechargeTestCase(TestCase):
    ...
    def test_apply_paid_order_skips_credit_when_recharge_record_already_exists(self):
        self._create_user(22)
        order = PaymentRechargeOrder(
            order_no="ALP202605160022",
            payment_channel="alipay",
            user_id=22,
            agent_id=None,
            site_scope="platform",
            source_host="www.example.com",
            return_url_snapshot="https://www.example.com/user/recharge?order_no=ALP202605160022",
            amount_cny=Decimal("10.00"),
            credited_usd=Decimal("50.000000"),
            agent_settlement_rate=Decimal("10.000000"),
            agent_income_cny=Decimal("0.00"),
        ...
        self.db.add(order)
        balance = self.db.query(UserBalance).filter(UserBalance.user_id == 22).first()
        balance.balance = Decimal("50.000000")
        balance.total_recharged = Decimal("50.000000")
        self.db.add(ConsumptionRecord(
            user_id=22,
