...
- 创建请求发送期间显示“正在提交视频任务”。
- 获取任务 ID 后，页面显示排队/生成状态、上游进度、已等待时间和任务 ID。
- 上游没有返回 `progress` 时不显示虚假的百分比，但仍持续更新状态与耗时。
- 每次轮询响应会同步到“原始响应”，便于确认任务当前状态。
- 正常视频任务通常需要 1 至数分钟；页面最多轮询 20 分钟，超时后会保留任务 ID。
...
生产环境中，已经失效但仍绑定 Grok 视频模型的旧渠道会造成创建请求先失败再切换。升级脚本 `backend/sql/upgrade_disable_legacy_grok_video_mapping_20260714.sql` 仅停用 `zz.211b` 的 Grok 视频映射，不影响该渠道的文本模型映射。
