...
- 限额套餐额度按北京时间滚动刷新，以当前 `user_subscription.start_time` 为刷新锚点；未选择周期时为 24 小时，选择多日周期后按所选天数刷新，不按自然日 0 点刷新。
- 管理端开通、代理端开通、用户端在线购买都复用同一套 `SubscriptionService` 周期计算和扣费逻辑。
- 用户端 `/user/balance` 套餐卡片展示下次刷新时间，来源于当前周期 `cycle_end_at`。
- 用户可在 `/user/balance` 为当前尚未设置周期的套餐选择 1、3、5 或 7 天刷新一次额度。套餐完整有效期小于 7 天按日卡处理，7 天至不满 30 天按周卡处理，30 天及以上按月卡处理；对应最多可选择 1、3、7 天，选择后不可修改，且选项按开通时的完整有效期计算，不受当前剩余时间影响。
- 多日周期额度按“每日额度 × 当前周期实际有效天数”计算，套餐到期前不足完整周期时按剩余天数折算。
- 请求扣费始终优先消耗套餐周期额度，套餐不足时仅对未覆盖部分扣用户余额；套餐和余额均不足时拒绝请求。
