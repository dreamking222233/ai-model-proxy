      ...
      <a-tabs v-model="activeTab">
        <a-tab-pane key="relations" tab="推广关系">
          <div class="toolbar">
            ...
            <a-select v-model="relationFilters.site_scope" allowClear placeholder="站点类型" style="width: 130px" @change="handleRelationSearch">
              <a-select-option value="platform">直营站</a-select-option>
              <a-select-option value="agent">代理站</a-select-option>
            </a-select>
          ...
          >
            ...
            <template slot="site" slot-scope="text, record"><a-tag :color="record.site_scope === 'agent' ? 'orange' : 'purple'">{{ record.site_scope === 'agent' ? '代理站' : '直营站' }}</a-tag></template>
        ...
        <a-tab-pane key="rewards" tab="返现明细">
          <div class="toolbar">
            ...
            <a-input v-model="rewardFilters.agent_id" allowClear placeholder="代理ID，直营填0" style="width: 140px" @pressEnter="handleRewardSearch" />
          </div>
          <a-table
            row-key="id"
            ...
            :scroll="{ x: 1450 }"
            @change="handleRewardTableChange"
          >
            <template slot="promoter" slot-scope="text, record"><div class="stack-text"><strong>{{ record.promoter_username }}</strong><small>{{ record.promoter_email }}</small></div></template>
            <template slot="invited" slot-scope="text, record"><div class="stack-text"><strong>{{ record.invited_username }}</strong><small>{{ record.invited_email }}</small></div></template>
...
export default {
  ...
  data() {
    return {
      ...
      relations: [],
      rewards: [],
      relationFilters: { keyword: '', site_scope: undefined, has_recharged: undefined, agent_id: '' },
      rewardFilters: { keyword: '', reward_asset_type: undefined, recharge_type: undefined, agent_id: '' },
      ...
      relationColumns: [
        ...
        { title: '站点', key: 'site', width: 110, scopedSlots: { customRender: 'site' } },
        { title: '代理', dataIndex: 'agent_name', key: 'agent_name', width: 160 },
  ...
  methods: {
    ...
    async fetchSummary() {
      ...
      try {
        const res = await getAdminPromotionSummary()
        this.summary = res.data || { relation_count: 0, reward_count: 0, reward_amount_total: 0 }
      } finally { this.loadingSummary = false }
    },
    agentIdValue(value) { return value === '' || value === undefined || value === null ? undefined : Number(value) },
    async fetchRelations() {
      this.loadingRelations = true
      try {
        const res = await listAdminPromotionRelations({ page: this.relationPagination.current, page_size: this.relationPagination.pageSize, keyword: this.relationFilters.keyword || undefined, site_scope: this.relationFilters.site_scope || undefined, has_recharged: this.relationFilters.has_recharged || undefined, agent_id: this.agentIdValue(this.relationFilters.agent_id) })
        const data = res.data || {}
    ...
    async fetchRewards() {
      ...
      try {
        const res = await listAdminPromotionRewards({ page: this.rewardPagination.current, page_size: this.rewardPagination.pageSize, keyword: this.rewardFilters.keyword || undefined, reward_asset_type: this.rewardFilters.reward_asset_type || undefined, recharge_type: this.rewardFilters.recharge_type || undefined, agent_id: this.agentIdValue(this.rewardFilters.agent_id) })
        const data = res.data || {}
...
.promotion-manage-page { padding: 24px; }
