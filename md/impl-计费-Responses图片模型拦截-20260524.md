...
修复图片模型和 Responses 图片工具可通过 `/v1/responses` 绕过图片余额校验的问题。Responses 入口现在只允许文本能力继续进入文本额度预检和上游转发；图片模型、图片上游映射必须使用图片专用接口，Responses 原生 `image_generation` 工具会在转发前被剔除。
...
- `backend/app/services/proxy_service.py`
  ...
  - 新增 `_remove_responses_image_generation_tool`，当 Responses `tools` 包含 `type=image_generation` 时从请求中剔除该工具，并清理强制选择该工具的 `tool_choice`。
...
- `backend/app/test/test_subscription_compatibility.py`
  ...
  - 新增单测覆盖文本模型携带 Responses `image_generation` 工具时不会被拒绝，但该工具会在转发前被剔除。
  - 新增单测覆盖强制选择 `image_generation` 工具的 `tool_choice` 会被清理。
...
对于文本模型携带 Responses 原生 `image_generation` 工具的情况，公共入口会直接从请求体中移除该工具，避免请求进入上游 Responses 后由上游工具执行生图；如果客户端只是普通对话但默认携带该工具，文本请求仍可继续。
