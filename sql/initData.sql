...
CREATE TABLE `agent` (
  ...
  `custom_recharge_rate_enabled` smallint NOT NULL DEFAULT '0' COMMENT '代理自定义充值比例权限 1=开启 0=关闭',
  `custom_recharge_rate` decimal(12,6) NOT NULL DEFAULT '5.000000' COMMENT '代理站用户统一充值比例',
...
CREATE TABLE `channel` (
  ...
  `provider_variant` varchar(32) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'default' COMMENT '渠道子类型: default/openai-image-compatible/openai-image-native-size/openai-image-modelinvoke/geek2api-image/cpa-grok-video/zz1cc-video/google-official/google-vertex-image',
...
CREATE TABLE `consumption_record` (
  ...
  `cache_creation_cost` decimal(12,6) DEFAULT '0.000000',
  ...
  `cache_read_price_per_million_snapshot` decimal(12,6) DEFAULT NULL COMMENT '缓存读取每百万Token实际单价快照',
  `cache_creation_price_per_million_snapshot` decimal(12,6) DEFAULT '0.000000' COMMENT '缓存创建每百万Token单价快照',
  ...
  `context_price_multiplier_snapshot` decimal(12,6) DEFAULT '1.000000' COMMENT '长上下文价格倍率快照',
...
CREATE TABLE `payment_recharge_order` (
  ...
  `user_recharge_rate` decimal(12,6) NOT NULL DEFAULT '0.000000' COMMENT '下单时实际用户充值比例快照',
...
CREATE TABLE `unified_model` (
  ...
  `cache_read_price_per_million` decimal(12,6) DEFAULT NULL COMMENT '每百万缓存读取Token单价(美元)，为空时按输入价格10%',
  `cache_creation_price_per_million` decimal(12,6) NOT NULL DEFAULT '0.000000' COMMENT '每百万缓存创建Token单价(美元)',
  ...
  `long_context_billing_enabled` tinyint NOT NULL DEFAULT '0' COMMENT '是否启用长上下文2倍计费',
  `long_context_token_threshold` int NOT NULL DEFAULT '262144' COMMENT '长上下文双倍计费Token阈值',
