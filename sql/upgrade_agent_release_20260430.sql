-- ============================================================
-- 代理端完整上线升级 SQL 汇总
-- 文件名：sql/upgrade_agent_release_20260430.sql
-- 用途：正式环境上线代理端相关功能时统一执行，避免漏执行分散 SQL。
--
-- 执行方式：
--   在项目根目录执行：
...
--
-- 注意：
--   1. 本文件依赖 MySQL 客户端 SOURCE 命令，因此需要从项目根目录执行。
--   2. 各子 SQL 均按幂等方式编写，可重复执行。
--   3. 如果服务器执行路径不是项目根目录，请改用绝对路径 SOURCE。
-- ============================================================

SOURCE backend/sql/upgrade_agent_portal_20260425.sql;
